...
check_cont <- function(df, var, group = NULL) {
  if (is.null(group)) {
    ...
    desc_plot <- ggtexttable(desc_statby(df, measure.var = var,
                                         grps = NULL)[, c("min","max","median","mean","iqr","sd")], 
                             rows = NULL) %>%
      tab_add_title(text = "Description statistics", face = "bold")
    
    shapiro_plot <- ggtexttable(df %>%
    ...
    box_plot <- ggboxplot(df, x = group, y = var, fill = group, palette = "jco") +
      theme_minimal() +
      labs(title = paste("Box plot of", var))
    
    desc_plot <- ggtexttable(desc_statby(df, measure.var = var,
                                         grps = group)[, c("group", "min","max","median","mean","iqr","sd")], 
                             rows = NULL) %>%
      tab_add_title(text = "Description statistics", face = "bold")
  ...
  ggarrange(dens_plot, qq_plot, box_plot, 
...
# ------------------------------------------------------------------------------
find_best_measurement <- function(dates, values, return = "dates", return_method = "closest") {
...
fix_height <- function(height, date) {
  ...
  if (length(height) > 1) {
    ...
    if (is.na(df$height[1])) {
      right_index <- 2
      while (is.na(df$height[right_index]) &
    ...
    for (i in 2:length(height)) {
      if (!is.na(df$height[i])) {
        ...
        if (is.na(df$height[left_index])) {
          while (is.na(df$height[left_index]) &
                 left_index > 1) {
    ...
    if (is.na(df$height[length(height)])) {
      ...
      if (difftime(df$date[length(height)], df$date[left_index], units = "days") < 186) {
    ...
    for (i in 2:mis_n) {
      ...
      if (is.na(df$height[i])) {
        ...
        if (!is.na(df$height[right_index]) &
          ...
          } else if (difftime(df$date[right_index], df$date[left_index], units = "days") < 186) {
...
# ------------------------------------------------------------------------------
visualize_lme_model <- function(model, name, data = final_df_long, 
                                min_year = NULL, max_year = NULL,
                                default_sex = "Female", default_age = NULL) {
  ...
  interaction_plot <- plot_model(model, type = "int") +
    labs(
      title = paste("Predicted", name, "Over Time (P:", 
  ...
  year_effect_plot <- plot(effects_year) +
    annotate("text", x = max(effects_year$x), y = max(effects_year$predicted), 
             label = annotate_significance("year"), size = 4, hjust = 1.1) +
    labs(title = paste("Predictors' Marginal Effect on", name, "SDS"),
         y = paste("Predicted", name))
  
  group_effect_plot <- plot(effects_group) +
    annotate("text", x = 2, y = max(effects_group$predicted), 
             label = annotate_significance("groupCase"), size = 4, hjust = 1.1) +
    labs(title = "",
         y = "")
  
  sex_effect_plot <- plot(effects_sex) +
    annotate("text", x = 2, y = max(effects_sex$predicted), 
             label = annotate_significance("sexMale"), size = 4, hjust = 1.1) +
    labs(title = "",
         y = "")
  
  age_effect_plot <- plot(effects_age) +
    annotate("text", x = max(effects_age$x), y = max(effects_age$predicted), 
             label = annotate_significance("age"), size = 4, hjust = 1.1) +
    labs(title = "",
         y = "")
  
  # Combine all marginal effect plots using patchwork
  marginal_effects_plots <- (year_effect_plot | group_effect_plot | sex_effect_plot | age_effect_plot)
