...
df_raw <- read_clip_tbl() %>% 
  ...
  mutate(across(contains("_result_"), ~ ifelse(as.numeric(.x) > 0, as.numeric(.x), NA))) %>% 
  
  # Convert specified binary columns to logical (TRUE when value equals 1)
  mutate(across(c("repeated_surgery_after_index", "dm2", "hyperlipidemia_5",
                  "pre_dm", "htn", "lipid", "anti_depress_drugs_before_index",
                  "anti_depress_drugs_after_index"), ~ .x == 1)) %>% 
  ...
  mutate(
    folic_acid_diff = ifelse(
      is.na(folic_acid_last_sample_result_before_index) | is.na(avg_value_of_folic_acid_in_5th_year_from_index), 
      NA, 
      avg_value_of_folic_acid_in_5th_year_from_index - folic_acid_last_sample_result_before_index
    ),
    .after = folic_acid_last_sample_result_before_index
  ) %>%
  mutate(
    hemoglobin_diff = ifelse(
      is.na(hemoglobin_last_sample_result_before_index) | is.na(avg_value_of_hemoglobin_in_5th_year_from_index), 
      NA, 
      avg_value_of_hemoglobin_in_5th_year_from_index - hemoglobin_last_sample_result_before_index
    ),
    .after = hemoglobin_last_sample_result_before_index
  ) %>%
  mutate(
    tsh_diff = ifelse(
      is.na(tsh_last_sample_result_before_index) | is.na(avg_value_of_tsh_in_5th_year_from_index), 
      NA, 
      avg_value_of_tsh_in_5th_year_from_index - tsh_last_sample_result_before_index
    ),
    .after = tsh_last_sample_result_before_index
  ) %>%
  mutate(
    vitamin_b12_diff = ifelse(
      is.na(vitamin_b12_last_sample_result_before_index) | is.na(avg_value_of_vitamin_b12_in_5th_year_from_index), 
      NA, 
      avg_value_of_vitamin_b12_in_5th_year_from_index - vitamin_b12_last_sample_result_before_index
    ),
    .after = vitamin_b12_last_sample_result_before_index
  ) %>%
  mutate(
    vitamin_d_diff = ifelse(
      is.na(vitamin_d_last_sample_result_before_index) | is.na(avg_value_of_vitamin_d_in_5th_year_from_index), 
      NA, 
      avg_value_of_vitamin_d_in_5th_year_from_index - vitamin_d_last_sample_result_before_index
    ),
