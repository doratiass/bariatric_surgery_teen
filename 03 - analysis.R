...
norm_vars <- c("weight_0", "height_0",
               "hemoglobin_0", "vitamin_d_0")
...
tbl_1_vars <- c("index_year", "age", "sex", "ses", "sector", "periphery", 
                "dm2", "pre_dm", "htn", "hyperlipidemia_5")
...
final_df_wide %>%
  ...
  tbl_summary(by = group,
              missing = "no",
              type = list(label_get("age") ~ 'continuous',
  ...
  add_p(test = list(
    ...
    all_of(vars_label(non_norm_vars[non_norm_vars %in% tbl_1_vars])) ~ "wilcox.test"
  )) -> tbl_1
...
plot_theme <- theme_minimal(base_size = 14) +  # Base font size
  theme(
    panel.grid.major = element_line(size = 0.2, linetype = "dashed", color = "gray80"),
...
bmi_plot_df %>%
  ...
  geom_line() +
  geom_point() +
  geom_line(aes(x = year, y = value, color = group, group = fake_id), 
            alpha = 0.1, data = bmi_plot_df) +
  ...
  labs(title = "Measurements over time", 
       x = "Follow up Year", 
       y = "Mean value", 
       color = "") +
...
ggsave(file.path("export", "fig_bmi.jpeg"), fig_bmi, 
       width = 30, height = 20, dpi = 300, background = "white", units = "cm")

## Visualize Laboratory Follow-Up Data -----------------------------------------
labs_plot_df <- final_df_long %>% 
  filter(!(test %in% c("bmi", "weight", "height", "folic_acid",
                       "bmi_sds", "weight_sds", "height_sds"))) %>%
  mutate(test = factor(case_when(
...
fig_labs <- labs_plot_df %>%
  ...
  geom_errorbar(aes(ymin = mean - 1.96 * se, ymax = mean + 1.96 * se), 
                width = 0.2, alpha = 0.7) +  # Semi-transparent error bars
  geom_text(data = labs_plot_annotated, 
            aes(label = p_mark, y = mean + 1.96 * se + 0.2), 
            color = "black", size = 4, fontface = "bold") +  # Bold annotations
...
ggsave(file.path("export", "fig_labs.jpeg"), fig_labs, 
       width = 30, height = 20, dpi = 300, background = "white", units = "cm")
...
height_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                     data = final_df_long %>% 
...
ggsave(file.path("export", "fig_lme_height.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## Weight Model (0-2 Years) -------------------------------------------------
# Fit a linear mixed-effects model for weight SDS for years <= 2
weight_0_2_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                         data = final_df_long %>% 
                           mutate(group = relevel(group, "Control")) %>%
                           filter(test == "weight_sds",
                                  year <= 2))
...
ggsave(file.path("export", "fig_lme_weight_0_2.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## Weight Model (2-5 Years) -------------------------------------------------
# Fit a linear mixed-effects model for weight SDS for years >= 2
weight_2_5_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                         data = final_df_long %>% 
                           mutate(group = relevel(group, "Control")) %>%
                           filter(test == "weight_sds",
                                  year >= 2))
...
ggsave(file.path("export", "fig_lme_weight_2_5.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## BMI Model (0-2 Years) ----------------------------------------------------
# Fit a linear mixed-effects model for BMI SDS for years <= 2
bmi_0_2_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                      data = final_df_long %>% 
                        mutate(group = relevel(group, "Control")) %>%
                        filter(test == "bmi_sds",
                               year <= 2))
...
ggsave(file.path("export", "fig_lme_bmi_0_2.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## BMI Model (2-5 Years) ----------------------------------------------------
# Fit a linear mixed-effects model for BMI SDS for years >= 2
bmi_2_5_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                      data = final_df_long %>% 
                        mutate(group = relevel(group, "Control")) %>%
                        filter(test == "bmi_sds",
                               year >= 2))
...
ggsave(file.path("export", "fig_lme_bmi_2_5.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## Hemoglobin Model ---------------------------------------------------------
# Fit a linear mixed-effects model for hemoglobin
hemoglobin_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                         data = final_df_long %>% 
                           mutate(group = relevel(group, "Control")) %>%
...
ggsave(file.path("export", "fig_lme_hem.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## TSH Model --------------------------------------------------------------
# Fit a linear mixed-effects model for TSH
tsh_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                  data = final_df_long %>% 
                    mutate(group = relevel(group, "Control")) %>%
                    filter(test == "tsh"))
...
ggsave(file.path("export", "fig_tsh.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## Vitamin B12 Model -------------------------------------------------------
# Fit a linear mixed-effects model for Vitamin B12
vitamin_b12_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                          data = final_df_long %>% 
                            mutate(group = relevel(group, "Control")) %>%
...
ggsave(file.path("export", "fig_lme_b12.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## Folic Acid Model --------------------------------------------------------
# Fit a linear mixed-effects model for folic acid
folic_acid_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                         data = final_df_long %>% 
                           mutate(group = relevel(group, "Control")) %>%
...
ggsave(file.path("export", "fig_lme_folic.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")

## Vitamin D Model ---------------------------------------------------------
# Fit a linear mixed-effects model for Vitamin D
vitamin_d_model <- lmer(value ~ year * group + sex + age + (1 | fake_id), 
                        data = final_df_long %>% 
                          mutate(group = relevel(group, "Control")) %>%
...
ggsave(file.path("export", "fig_lme_vit_d.jpeg"), last_plot(), 
       width = 30, height = 15, dpi = 300,
       background = "white", units = "cm")
...
model_names <- c("height", "weight_0_2", "weight_2_5", "bmi_0_2", "bmi_2_5", 
                 "hemoglobin", "tsh", "vitamin_b12", "vitamin_d")

# Extract coefficients from each model into a list of tables
model_tables <- map(model_names, ~ extract_coefficients(get(paste0(. , "_model")), .))
...
gtsave(coef_table_gt, file = file.path("export", "coef_table.docx"), to = "docx")